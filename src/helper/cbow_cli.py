...
DEFAULT_CONFIG_PATH = Path("configs/config_cbow.yaml")
...
def config_defaults(config: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "save_path": Path(
            str(output_cfg.get("save_path", config.get("save_path", "artifacts/cbow/best_model.pt")))
        ),
        "metrics_path": Path(
            str(
                output_cfg.get(
                    "metrics_path",
                    config.get("metrics_path", "artifacts/cbow/metrics.json"),
                )
            )
        ),
...
def add_cli_args(parser: argparse.ArgumentParser, defaults: dict[str, object] | None = None) -> None:
    ...
    parser.add_argument("--save-path", type=Path, default=defaults.get("save_path", Path("artifacts/cbow/best_model.pt")))
    parser.add_argument(
        "--metrics-path",
        type=Path,
        default=defaults.get("metrics_path", Path("artifacts/cbow/metrics.json")),
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train CBOW model for embedding learning.")
