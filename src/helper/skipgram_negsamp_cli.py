...
DEFAULT_CONFIG_PATH = Path("configs/config_skipgram_negsamp.yaml")
...
def config_defaults(config: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "context_size": int(model_cfg.get("context_size", config.get("context_size", 2))),
        ...
        "num_negatives": int(optim_cfg.get("num_negatives", config.get("num_negatives", 10))),
        "negative_sampling_power": float(
            optim_cfg.get("negative_sampling_power", config.get("negative_sampling_power", 0.75))
        ),
        ...
        "save_path": Path(
            str(
                output_cfg.get(
                    "save_path",
                    config.get("save_path", "artifacts/skipgram_negsamp/best_model.pt"),
                )
            )
        ),
        "metrics_path": Path(
            str(
                output_cfg.get(
                    "metrics_path",
                    config.get("metrics_path", "artifacts/skipgram_negsamp/metrics.json"),
                )
            )
        ),
...
def add_cli_args(parser: argparse.ArgumentParser, defaults: dict[str, object] | None = None) -> None:
    ...
    parser.add_argument("--context-size", type=int, default=defaults.get("context_size", 2), help="Window size on each side of center token.")
    ...
    parser.add_argument("--num-negatives", type=int, default=defaults.get("num_negatives", 10))
    parser.add_argument(
        "--negative-sampling-power",
        type=float,
        default=defaults.get("negative_sampling_power", 0.75),
        help="Exponent used to build unigram negative-sampling distribution.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-path",
        type=Path,
        default=defaults.get("save_path", Path("artifacts/skipgram_negsamp/best_model.pt")),
    )
    parser.add_argument(
        "--metrics-path",
        type=Path,
        default=defaults.get("metrics_path", Path("artifacts/skipgram_negsamp/metrics.json")),
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train Skip-gram model with negative sampling.")
