...
from src.helper.nplm_data import SPLITS, Vocabulary, create_provider
from src.helper.nplm_logging import run_with_tee_logging
from src.helper.skipgram_negsamp_cli import parse_args
...
class SkipGramDataset(Dataset[tuple[Tensor, Tensor]]):
    """Create (center_token, context_token) pairs for skip-gram training."""

    def __init__(self, token_ids: list[int], context_size: int) -> None:
        if context_size <= 0:
            raise ValueError("context_size must be > 0")
        if len(token_ids) <= (2 * context_size):
            raise ValueError(
                f"Not enough tokens ({len(token_ids)}) for context_size={context_size}"
            )
        self.token_ids = token_ids
        self.context_size = context_size
        self.centers_count = len(token_ids) - (2 * context_size)
        self.contexts_per_center = 2 * context_size

    def __len__(self) -> int:
        return self.centers_count * self.contexts_per_center

    def __getitem__(self, idx: int) -> tuple[Tensor, Tensor]:
        center_offset = idx // self.contexts_per_center
        context_slot = idx % self.contexts_per_center

        center_index = self.context_size + center_offset
        if context_slot < self.context_size:
            context_index = center_index - self.context_size + context_slot
        else:
            context_index = center_index + 1 + (context_slot - self.context_size)

        center_token = self.token_ids[center_index]
        context_token = self.token_ids[context_index]
        return (
            torch.tensor(center_token, dtype=torch.long),
            torch.tensor(context_token, dtype=torch.long),
        )


def build_skipgram_dataloaders(
    ...
    datasets = {
        split: SkipGramDataset(token_ids=token_ids, context_size=context_size)
...
class SkipGramNegativeSamplingTrainer:
    def __init__(
        self,
        model: SkipGram,
    ...
    def _batch_loss(self, center: Tensor, positive_context: Tensor) -> Tensor:
        center = center.to(self.device, non_blocking=True)
        positive_context = positive_context.to(self.device, non_blocking=True)

        negatives = sample_negatives(
            probs=self.negative_probs,
            positive_targets=positive_context.detach().cpu(),
            num_negatives=self.num_negatives,
            generator=self.generator,
        ).to(self.device, non_blocking=True)

        center_embed = self.model.embedding(center)  # [B, E]
        out_weight = self.model.output.weight  # [V, E]
        out_bias = self.model.output.bias  # [V]

        pos_weight = out_weight[positive_context]  # [B, E]
        pos_bias = out_bias[positive_context]  # [B]
        pos_logits = (center_embed * pos_weight).sum(dim=1) + pos_bias  # [B]

        neg_weight = out_weight[negatives]  # [B, K, E]
        neg_bias = out_bias[negatives]  # [B, K]
        neg_logits = torch.bmm(neg_weight, center_embed.unsqueeze(2)).squeeze(2) + neg_bias  # [B, K]
    ...
    def train_epoch(self, loader: DataLoader[tuple[Tensor, Tensor]]) -> Metrics:
        ...
        for center, context in loader:
            ...
            loss = self._batch_loss(center, context)
            loss.backward()
            self.optimizer.step()
...
def run(args: object) -> int:
    ...
    dataloaders = build_skipgram_dataloaders(
