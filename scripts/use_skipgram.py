...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory to save embedding plot images (default depends on checkpoint type).",
    )
    parser.add_argument(
        "--export-embeddings",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.plot_k > 0:
        ...
        pca_path = plot_dir / f"{checkpoint_stem}_k{chosen_ids.numel()}_pca.png"
        dist_path = plot_dir / f"{checkpoint_stem}_k{chosen_ids.numel()}_dist.png"
    ...
    if args.plot_training_curves:
        ...
        curves_path = args.curves_path or (
            plot_dir / f"{checkpoint_path.stem}_training_curves.png"
        )
